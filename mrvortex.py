...
__author__ = "Mr.Vortex"
__license__ = "Mr.Vortex"
__version__ = "1.0.0"
...
def print_to_file(filename, unique_list_finished):
    ...
    f.close()
    print(
        "[+] PASS LİST OLUŞTURULMUŞTUR \033[1;31m"
        + filename
        + "\033[1;m, BU PROGRAMIN OLDUGU DOSYAYA KAYDEDİLMİŞTİR \033[1;31m"
        + str(lines)
        + " KELİME.\033[1;m"
    )
    inspect = input("> PASS LİSTİ OKUTMAK İSTERMİSİNİZ (Y/n) : ").lower()
    if inspect == "y":
        try:
    ...
    print(
        "[+] PASS LİSTİNİZ BU PROGRAMIN OLDUĞU DOSYAYA \033[1;31m"
        + filename
        + "\033[1; B BAŞARIYLA KAYDEDİLMİŞTİR."
    )
...
def improve_dictionary(file_to_open):
    ...
    if leetmode == "y":
        for (
            ...
            unique_lista
        ):  # if you want to add more leet chars, you will need to add more lines in mrvortex.cfg too...
    ...
    print_to_file(file_to_open + ".mrvortex.txt", unique_list_finished)
...
def interactive():
    ...
    print("\rPassList Oluşturmak İçin Aşagıdaki Sorularda Hedef Kişinin Bilgilerini Giriniz")
    print("Hedef Kişinin bilmediğiniz Bilgilerini (enter) Tuşuna Basarak Boş Bırakabilirsiniz. \r\n")
    ...
    name = input("> Hedef Kişinin İSMİ: ").lower()
    while len(name) == 0 or name == " " or name == "  " or name == "   ":
        print("\r\n[-] Hedef Kişinin İsmi Zorunludur(Lütfen Giriniz)")
        name = input("> Hedef Kişinin İSMİ: ").lower()
    profile["name"] = str(name)

    profile["surname"] = input("> Hedef Kişinin Soyadı: ").lower()
    profile["nick"] = input("> Hedef Kişinin Takma Adı: ").lower()
    birthdate = input("> Hedef Kişinin Doğum Tarihi Örn:(DDMMYYYY): ")
    while len(birthdate) != 0 and len(birthdate) != 8:
        print("\r\n[-] Lütfen Gün Ay Yıl Olarak Örnekteki Gibi Birleşik Şekilde Yazınız")
        birthdate = input("> Hedef Kişinin Doğum Tarihi Örn:(DDMMYYYY): ")
    profile["birthdate"] = str(birthdate)

    print("\r\n")

    profile["wife"] = input("> Hedef Kişinin Eşinin) İSMİ: ").lower()
    profile["wifen"] = input("> Hedef Kişinin Eşinin) TAKMA ADI: ").lower()
    wifeb = input("> EŞİNİN) DOĞUM TARİHİ Örn:(DDMMYYYY): ")
    ...
    profile["kid"] = input("> Hedef Kişinin Çoçuğunun Adı: ").lower()
    profile["kidn"] = input("> Hedef Kişinin Çocuğunun Takma Adı: ").lower()
    kidb = input("> Hedef Kişinin Çocuğunun Doğum Tarihi Örn:(DDMMYYYY): ")
    ...
    profile["pet"] = input("> Hedef Kişinin Evcil Hayvanı Varsa İsmi: ").lower()
    profile["company"] = input("> Hedef Kişinin Memleketi Veya Yaşadığı Sehrin plaka Numarası: ").lower()
    print("\r\n")

    profile["words"] = [""]
    words1 = input(
        "> Hedef Kişinin Hakkında Daha Fazla Bilgi Eklemek İstermisiniz Y/[N]: "
    ).lower()
    words2 = ""
    if words1 == "y":
        words2 = input(
            "> Lütfen Kelimeleri Virgül İle Ayırarak Giriniz. Örn:[Bilginiz, Bilginiz, Bilginiz] "
        ).replace(" ", "")
    profile["words"] = words2.split(",")

    profile["spechars1"] = input(
        "> Kelimelerin sonuna özel karakterler eklemek ister misiniz? Y/[N]: "
    ).lower()

    profile["randnum"] = input(
        "> Kelimelerin sonuna rastgele sayılar eklemek ister misiniz? Y/[N]:"
    ).lower()
    profile["leetmode"] = input("> Leet modu? (yani leet = 1337) Y/[N]: ").lower()
...
# the main function
def main():
    """Command-line interface to the mrvortex utility"""

    read_config(os.path.join(os.path.dirname(os.path.realpath(__file__)), "mrvortex.cfg"))
